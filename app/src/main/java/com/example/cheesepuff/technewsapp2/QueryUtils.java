    ...
    private static List<Technews> extractFeatureFromJson(String technewsJSON) {
        ...
        try {
            ...
            for (int i = 0; i < technewsArray.length(); i++) {
                ...
                // for that tech news.
                String  publication = currentTechnews.optString(PUBLICATION);
                String headline = currentTechnews.optString(HEADLINE);

                // Extract the value for the key called "time"
                String date = formatDate(currentTechnews.optString(DATE));

                // Extract the value for the key called "url"
                String url = currentTechnews.optString(URL);
